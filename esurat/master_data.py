...
def _load_master_state(app: Flask) -> dict[str, Any]:
    ...
    if guru_records is None:
        guru_records = _read_json_list(data_dir / "guru.json", "guru")
    if murid_records is None:
        murid_records = _read_json_list(data_dir / "murid.json", "murid")
    if kode_records is None:
        kode_records = _read_json_list(data_dir / "kode_arsip.json", "kode arsip")
