...
Panduan ini ditujukan kepada operator Tata Usaha dan administrator teknis SMAN 2 Wonosari. Versi audit: 23 Agustus 2026.
...
| --- | --- |
| Operator TU | Memilih orang yang benar, mengisi surat, memeriksa ringkasan dan DOCX, menyimpan hasil di lokasi arsip resmi |
| Pemeriksa/pejabat berwenang | Memeriksa substansi, nomor, penandatangan, dan kelayakan terbit |
| Administrator teknis | Menjaga service, autentikasi, TLS, data master, template, backup, restore, dan log insiden |
...
Satu akun bersama dapat menyulitkan audit. Bila aplikasi mulai dipakai lebih luas, sediakan akun individual atau integrasi identitas sebagai backlog prioritas.
...
Repositori yang diaudit masih melacak data pribadi dan memiliki remote GitHub. Administrator wajib menyelesaikan tindakan P0 pada [Audit Produksi](AUDIT_PRODUKSI.md#p0-data-pribadi-di-git-dan-github) sebelum rollout.
...
- Pastikan respons berstatus sehat, pemeriksaan database/template lulus, dan jumlah guru, siswa, serta kode arsip masuk akal.
- Jika status tidak sehat atau jumlah data berubah tanpa jadwal, hentikan penerbitan surat dan hubungi administrator.
...
- Gunakan nomor manual hanya jika pejabat pengelola nomor telah memberikan nomor tersebut. Aplikasi menolak nomor yang sudah dipakai, tetapi tetap diperlukan kebijakan pencatatan di luar aplikasi bila ada sistem nomor lain.
...
| HTTP 403 / CSRF | Refresh halaman atau login ulang, lalu isi ulang dengan hati-hati |
...
Untuk satu komputer lokal, biarkan `ESURAT_HOST=127.0.0.1` dan jangan aktifkan akses jarak jauh. Untuk LAN, pertahankan backend di `127.0.0.1:5000`, aktifkan username + hash password + secret stabil, set `ESURAT_HTTPS=1`, dan letakkan reverse proxy HTTPS pada port 443. Firewall tidak boleh membuka port 5000. Batasi `/healthz` di proxy/firewall.
...
7. Periksa `/healthz`, cocokkan jumlah, cari beberapa guru/siswa/kode, lalu generate satu surat QA sesuai prosedur.
...
7. Pastikan seluruh automated test lulus. Buka ketujuh file di `qa/generated/` dan periksa Word serta Print Preview. Hapus/arsipkan hasil QA sesuai kebijakan karena isinya berasal dari data master produksi meskipun database suratnya sementara.
...
`manifest.json` mencatat `layout_version`, mapping tujuan restore, path, ukuran, dan SHA-256 setiap berkas. Verifikasi hash sebelum restore. Backup default tidak mencakup source code, konfigurasi secret, atau template DOCX; simpan rilis aplikasi dan template secara terpisah serta privat.
...
5. Pulihkan hanya komponen yang diperlukan:
   ...
   - `guru.json`, `murid.json`, dan `kode_arsip.json` ke `data/master/` bila master rusak;
   - `surat_smada.db` ke `data/runtime/` hanya bila database memang harus dipulihkan;
   - isi `source/` ke `data/source/` hanya bila workbook sumber juga harus dikembalikan.
...
8. Periksa `/healthz`, jumlah master, beberapa pencarian, login, dan Riwayat.
